...
def process_svs(twixObj, base_name_out, name_in, dataKey, dim_overrides, remove_os, quiet=False, verbose=False):
    ...
    # AG 03/25/2025 - Support for Siemens Update to 'smm_svs_isthmus_gls'
    elif any(x in twixObj['hdr']['Meas'][('tSequenceFileName')] for x in ('smm_svs_herc_hyper', 'svs_isthmus')):
        ...
        for ii, hyp_name in enumerate(hyp_names):                                           # Iterate over Subscans
            print(f'{ii:3d} {hyp_name:<20}', end='\r')
            reord_data_, meta_obj_, dim_tags_ = smm_svs_herc_hyper(twixObj,
                                                                   reord_data,
                                                                   meta_obj,
                                                                   dim_tags,
                                                                   subseq=ii,
                                                                   subseq_name=hyp_name)
