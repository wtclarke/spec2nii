...
from spec2nii.dcm2niiOrientation.orientationFuncs import (
...
def convert_rda(rda_path, fname_out, verbose):
    ...
    if (
        ...
        * int(hdr['CSIMatrixSize[2]'])
        ...
        raise MRSINotHandledError(
            'MRSI is currently not handled in the RDA format. Test data needed.'
        )
    ...
    imageOrientationPatient = np.asarray(
        [
            [
                _locale_float(hdr['RowVector[0]']),
                _locale_float(hdr['ColumnVector[0]']),
            ],
            [
                _locale_float(hdr['RowVector[1]']),
                _locale_float(hdr['ColumnVector[1]']),
            ],
            [
                _locale_float(hdr['RowVector[2]']),
                _locale_float(hdr['ColumnVector[2]']),
            ],
        ]
    ).T
    ...
    currNiftiOrientation = dcm_to_nifti_orientation(
    ...
    return [
        gen_nifti_mrs_hdr_ext(
            data_cmplx, dwelltime, meta, currNiftiOrientation.Q44, no_conj=True
        ),
    ], [
        name,
    ]
...
def extractRdaMetadata(hdr):
    ...
    obj = Hdr_Ext(_locale_float(hdr['MRFrequency']), hdr['Nucleus'])
    ...
    obj.set_standard_def(
        'ExcitationFlipAngle', _locale_float(hdr['FlipAngle'])
    )
    # 'TxOffset'
    # 'VOI'
    # 'WaterSuppressed'
